...
const login = {
  ...
  async resolve(parent, args) {
    ...
    if (!user || user.password !== args.password) {
      throw new Error(
        "Password incorrect or user with this email does not exist"
      );
    }
  ...
  args: {
    ...
    questions: {
      type: new GraphQLNonNull(
        new GraphQLList(new GraphQLNonNull(QuestionInputType))
      ),
    },
  },
  async resolve(parent, args) {
...
const submitQuiz = {
  ...
  args: {
    answers: {
      type: new GraphQLNonNull(
        new GraphQLList(new GraphQLNonNull(AnswerInputType))
      ),
    },
